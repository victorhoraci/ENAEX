            ...
            if sol.empty:
                ...
                if "Rango días solped" in sol.columns:
                    ...
                    st.plotly_chart(barras(conteo, {}, "Antigüedad de las solped", True),
                                    use_container_width=True)
                with st.expander("🏭 Proveedores sugeridos para estas solped"):
                    st.caption("Del historial de compra, mejor a peor por TAT. "
                               "Útil para elegir a quién adjudicar la OC.")
                    tabla_proveedores_sugeridos(sol["Material"].tolist(), "sol_prov")
        with c2:
...
def tabla_proveedores_sugeridos(materiales, key_prefix):
    """
    Muestra, para una lista de materiales SIN gestión (sin OC ni solped), hasta
    5 proveedores sugeridos del historial, ordenados de mejor a peor por TAT.
    """
    try:
        pm = cargar_proveedores_por_material()
    except Exception:
        st.caption("No hay histórico de compras para sugerir proveedores.")
        return
    sug = pm[pm["Material"].astype(str).isin([str(m) for m in materiales])]
    if sug.empty:
        st.caption("No se encontraron proveedores en el historial para estos materiales.")
        return
    vista = sug.rename(columns={
        "Proveedor_Codigo": "Código proveedor", "Proveedor_Nombre": "Proveedor",
        "TAT_Material": "TAT del material (días)", "OTIF_Proveedor": "OTIF proveedor %",
        "Compras": "Compras históricas"})
    cols = [c for c in ["Material", "Código proveedor", "Proveedor",
                        "TAT del material (días)", "OTIF proveedor %",
                        "Compras históricas", "Ultima_compra"] if c in vista.columns]
    vista = vista[cols].rename(columns={"Ultima_compra": "Última compra"})
    vista = buscar_en_tabla(vista, f"{key_prefix}_busca")
    st.dataframe(vista, use_container_width=True, hide_index=True)
    st.caption("Ordenados de mejor a peor por **TAT** (menor = mejor). "
               "El OTIF es el cumplimiento histórico del proveedor.")
...
def pagina_control():
    ...
    if criticos.empty:
        ...
        # Proveedores sugeridos para los críticos SIN gestión (sin OC ni solped)
        sin_gestion = criticos[criticos["Estado gestión"].isin(
            ["Sin gestión", "Validación", "Solped bloqueada"])]
        if not sin_gestion.empty:
            with st.expander(f"🏭 Proveedores sugeridos para los "
                             f"{len(sin_gestion)} materiales críticos sin OC/solped"):
                st.caption("Para materiales sin gestión activa, estos son los "
                           "proveedores del historial de compra (mejor a peor por TAT).")
                tabla_proveedores_sugeridos(sin_gestion["Material"].tolist(), "ctl_prov")
...
def cargar_analisis_proveedores():
    return analisis_proveedores()


@st.cache_data(show_spinner="Analizando compras por material…")
def cargar_analisis_materiales_compras():
    return analisis_materiales()


@st.cache_data(show_spinner="Cruzando material y proveedor…")
def cargar_compras_material_proveedor():
    return compras_material_proveedor()
...
def _cols_gasto(df):
    """Columnas de gasto por moneda presentes en el df."""
    return [c for c in df.columns if str(c).startswith("Gasto ")]


def pagina_proveedores():
    st.markdown(
        '<div class="hdr hdr-azul"><h1>🏭 Análisis de proveedores</h1>'
        '<p>Gasto por año y moneda · OTIF · TAT · contrato marco</p></div>',
        unsafe_allow_html=True,
    )
    try:
        prov = cargar_analisis_proveedores()
    except FileNotFoundError:
        st.error("Faltan los archivos de **histórico de compras (ME2M)** y/o **OTIF**. "
                 "Súbelos en la página **📥 Cargar archivos**.")
        return
    except Exception as e:
        st.error(f"No se pudo analizar: {e}")
        return
    if prov.empty:
        st.warning("No hay datos de compras para analizar.")
        return

    anios = sorted(prov["Año"].dropna().unique().astype(int), reverse=True)
    with st.sidebar:
        st.markdown("### Filtros")
        anio = st.selectbox("Año", ["Todos"] + [str(a) for a in anios], key="prov_anio")
        f_cm = filtro_multi(prov, "Tiene contrato marco", "Contrato marco", "prov_cm")
    ...
    datos = aplicar_filtro(datos, "Tiene contrato marco", f_cm)
    if datos.empty:
        st.warning("Ningún proveedor coincide con los filtros.")
        return

    cols_gasto = _cols_gasto(datos)

    # KPIs
    k = st.columns(4)
    k[0].metric("Proveedores", datos[["Proveedor_Codigo"]].drop_duplicates().shape[0])
    if "Gasto CLP" in datos.columns:
        k[1].metric("Gasto total CLP", _fmt_clp(datos["Gasto CLP"].sum(skipna=True)))
    if "OTIF" in datos.columns and datos["OTIF"].notna().any():
        k[2].metric("OTIF promedio", f"{datos['OTIF'].mean():.1f}%")
    con_cm = int((datos["Tiene contrato marco"] == "Sí").sum())
    k[3].metric("Con contrato marco", con_cm)

    st.markdown("---")

    # Top proveedores por gasto CLP
    if "Gasto CLP" in datos.columns:
        top = datos.groupby("Proveedor_Nombre", as_index=False)["Gasto CLP"].sum() \
                   .nlargest(15, "Gasto CLP")
        fig = go.Figure(go.Bar(
            y=top["Proveedor_Nombre"], x=top["Gasto CLP"], orientation="h",
            marker_color="#2E86DE",
            text=[_fmt_clp(v) for v in top["Gasto CLP"]], textposition="outside"))
        fig.update_layout(title="Top 15 proveedores por gasto (CLP)",
                          height=440, margin=dict(l=10, r=10, t=45, b=10),
                          plot_bgcolor="#fff", paper_bgcolor="#fff",
                          yaxis=dict(autorange="reversed"))
        st.plotly_chart(fig, use_container_width=True)

    # OTIF vs TAT (calidad del proveedor)
    if "OTIF" in datos.columns and datos["OTIF"].notna().any():
        val = datos[datos["OTIF"].notna() & datos["TAT prom (días)"].notna()]
        if not val.empty:
            fig = go.Figure(go.Scatter(
                x=val["TAT prom (días)"], y=val["OTIF"], mode="markers",
            ...
            fig.update_layout(title="Calidad del proveedor: OTIF vs TAT (arriba-izquierda = mejor)",
                              height=380, margin=dict(l=10, r=10, t=45, b=10),
                              plot_bgcolor="#fff", paper_bgcolor="#fff",
                              xaxis=dict(title="TAT promedio (días)"),
                              yaxis=dict(title="OTIF (%)"))
            st.plotly_chart(fig, use_container_width=True)
    ...
    datos["etiqueta_p"] = datos["Proveedor_Codigo"] + "  —  " + datos["Proveedor_Nombre"]
    et = st.selectbox("Proveedor", ["(ninguno)"] + sorted(datos["etiqueta_p"].unique()),
                      key="prov_sel")
    if et != "(ninguno)":
        cod = et.split("  —  ")[0].strip()
        hist_p = prov[prov["Proveedor_Codigo"] == cod].sort_values("Año")
        st.markdown(f"### {hist_p['Proveedor_Nombre'].iloc[0]}")
        st.caption(f"Código: {cod}")
        cols_show = ["Año", "Compras", "Materiales distintos", "TAT prom (días)",
                     "OTIF", "Tiene contrato marco"] + _cols_gasto(hist_p)
        cols_show = [c for c in cols_show if c in hist_p.columns]
        vista_p = hist_p[cols_show].copy()
        for c in _cols_gasto(vista_p):
            vista_p[c] = vista_p[c].apply(_fmt_clp)
        st.dataframe(vista_p, use_container_width=True, hide_index=True)

    st.markdown("---")

    # Tabla completa
    st.markdown("#### Todos los proveedores")
    cols = ["Proveedor_Codigo", "Proveedor_Nombre", "Año", "Compras",
            "Materiales distintos", "TAT prom (días)", "OTIF", "In_Full", "On_Time",
            "Tiene contrato marco"] + cols_gasto
    cols = [c for c in cols if c in datos.columns]
    vista = datos[cols].rename(columns={
        "Proveedor_Codigo": "Código", "Proveedor_Nombre": "Proveedor",
        "In_Full": "In Full %", "On_Time": "On Time %", "OTIF": "OTIF %"})
    ...
    st.download_button("⬇️  Descargar análisis de proveedores (CSV)",
                       data=vista.to_csv(index=False).encode("utf-8-sig"),
...
# ==========================================================================
#  PÁGINA · ANÁLISIS DE MATERIALES (compras)
# ==========================================================================
def pagina_materiales_compras():
    st.markdown(
        '<div class="hdr hdr-azul"><h1>📦 Análisis de compras por material</h1>'
        '<p>Gasto por año · proveedores usados · cantidad comprada</p></div>',
        unsafe_allow_html=True,
    )
    try:
        mat = cargar_analisis_materiales_compras()
        detalle = cargar_compras_material_proveedor()
    except FileNotFoundError:
        ...
        return
    if mat.empty:
        st.warning("No hay compras para analizar.")
        return

    # Datos del material desde el abastecimiento (criticidad, área, grupo, TAT)
    try:
        tabla_ab, _ = cargar_abastecimiento()
        info_mat = tabla_ab[["Material", "Criticidad texto", "Area", "Grupo de compras",
                             "TAT Promedio"]].drop_duplicates("Material")
    except Exception:
        info_mat = None

    cols_gasto = _cols_gasto(mat)

    st.markdown("#### Selecciona un material")
    mats = sorted(mat["Material"].unique())
    etiquetas = {}
    for m_ in mats:
        desc = mat[mat["Material"] == m_]["Descripción"].iloc[0]
        etiquetas[f"{m_}  —  {desc}"] = m_
    sel = st.selectbox("Material", ["(elige)"] + list(etiquetas.keys()), key="matc_sel")

    if sel == "(elige)":
        st.info("Elige un material para ver su análisis de compras.")
        # resumen global: top materiales por gasto
        if "Gasto CLP" in mat.columns:
            top = mat.groupby("Descripción", as_index=False)["Gasto CLP"].sum() \
                     .nlargest(15, "Gasto CLP")
            fig = go.Figure(go.Bar(
                y=top["Descripción"], x=top["Gasto CLP"], orientation="h",
                marker_color="#8E44AD",
                text=[_fmt_clp(v) for v in top["Gasto CLP"]], textposition="outside"))
            fig.update_layout(title="Top 15 materiales por gasto histórico (CLP)",
                              height=440, margin=dict(l=10, r=10, t=45, b=10),
                              plot_bgcolor="#fff", paper_bgcolor="#fff",
                              yaxis=dict(autorange="reversed"))
            st.plotly_chart(fig, use_container_width=True)
    ...
    material = etiquetas[sel]
    sub = mat[mat["Material"] == material].sort_values("Año")
    st.markdown(f"### {sub['Descripción'].iloc[0]}")

    # Datos del material
    if info_mat is not None:
        fila_i = info_mat[info_mat["Material"] == material]
        if len(fila_i):
            r = fila_i.iloc[0]
            c = st.columns(5)
            c[0].metric("Código", material)
            c[1].metric("Criticidad", str(r.get("Criticidad texto", "—")))
            c[2].metric("Área", str(r.get("Area", "—")))
            c[3].metric("Grupo compra", str(r.get("Grupo de compras", "—")))
            tat = r.get("TAT Promedio")
            c[4].metric("TAT prom", "—" if pd.isna(tat) else f"{tat:.0f} días")

    # Gasto por año
    if "Gasto CLP" in sub.columns:
        fig = go.Figure(go.Bar(x=sub["Año"].astype(str), y=sub["Gasto CLP"],
                               marker_color="#8E44AD",
                               text=[_fmt_clp(v) for v in sub["Gasto CLP"]],
                               textposition="outside"))
        fig.update_layout(title="Gasto por año (CLP)", height=320,
                          margin=dict(l=10, r=10, t=45, b=10),
                          plot_bgcolor="#fff", paper_bgcolor="#fff")
        st.plotly_chart(fig, use_container_width=True)

    # Detalle por proveedor y año (gasto y cantidad)
    st.markdown("#### Compras por proveedor y año")
    det = detalle[detalle["Material"] == material].sort_values(["Año", "Proveedor_Nombre"])
    if det.empty:
        st.info("Sin detalle por proveedor.")
    else:
        cols_d = ["Año", "Proveedor_Codigo", "Proveedor_Nombre", "Cantidad"] + _cols_gasto(det)
        cols_d = [c for c in cols_d if c in det.columns]
        vista_d = det[cols_d].rename(columns={
            "Proveedor_Codigo": "Código", "Proveedor_Nombre": "Proveedor"})
        for c in _cols_gasto(vista_d):
            vista_d[c] = vista_d[c].apply(_fmt_clp)
        st.dataframe(vista_d, use_container_width=True, hide_index=True)

    st.markdown("#### Resumen anual del material")
    cols_r = ["Año", "Cantidad comprada", "Proveedores distintos", "Compras"] + cols_gasto
    cols_r = [c for c in cols_r if c in sub.columns]
    vista_r = sub[cols_r].copy()
    for c in _cols_gasto(vista_r):
        vista_r[c] = vista_r[c].apply(_fmt_clp)
    st.dataframe(vista_r, use_container_width=True, hide_index=True)
...
PAGINAS = {
    ...
    "📦  Compras por material": pagina_materiales_compras,
